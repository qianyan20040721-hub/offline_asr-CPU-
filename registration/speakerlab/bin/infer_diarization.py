...
try:
    ...
    from ASR_pipeline.speaker.speakerlab.utils.config import Config

from ASR_pipeline.speaker.speakerlab.utils.builder import build
from ASR_pipeline.speaker.speakerlab.utils.utils import merge_vad, silent_print, download_model_from_modelscope, circle_pad
from ASR_pipeline.speaker.speakerlab.utils.fileio import load_audio
...
parser.add_argument('--wav', type=str,default=r"C:\Users\ROG\Downloads\test\zhaofeng-16k.wav",required=True, help='Input wavs')
parser.add_argument('--out_dir', type=str, required=True,default=r"D:\python_poj\3D_speaker\3D-Speaker", help='Out results dir')
...
class Diarization3Dspeaker():
    ...
    Usage:
        ASR_pipeline = Diarization3Dspeaker(device, include_overlap, hf_access_token)
        output = ASR_pipeline(input_audio) # input_audio can be a path to a WAV file, a NumPy array, or a PyTorch tensor
        print(output) # output: [[1.1, 2.2, 0], [3.1, 4.1, 1], ..., [st_n, ed_n, speaker_id]]
        ASR_pipeline.save_diar_output('audio.rttm') # or audio.json
    """
